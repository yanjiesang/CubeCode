...
{
    RecvMsgObject=new SystemV::Msg("/",2);  // 两个不同的进程进行消息队列通信，需要保持一样的ipc_key
    SendMsgObject=new SystemV::Msg("/",1);  // 两个不同的进程进行消息队列通信，需要保持一样的ipc_key
    ...
    pthread_t ReadShmTid;
    pthread_create(&ReadShmTid,NULL,ReadShmThreadRoutine,NULL);
    pthread_detach(ReadShmTid);
